...
describe('Basic user flow for PopupComponent', () => {
  beforeAll(async () => {
    await page.goto('http://127.0.0.1:5502/source/project/components/testingDeena.html'); // Adjust the path as needed
  });
  ...
  test('should close the popup when the overlay is clicked', async () => {
    ...
    await overlay.click();
    const displayStyle = await page.evaluate(popup => popup.style.display, popup);
  ...
  //   const shadowRootHandle = await popup.evaluateHandle(popup => popup.shadowRoot);
  ...
  //   // Check if the form is reset and popup is closed
  //   const titleValue = await titleInput.evaluate(input => input.value);
  //   const descriptionValue = await descriptionInput.evaluate(input => input.value);
  //   const dateValue = await dateInput.evaluate(input => input.value);
  //   const labelValue = await labelInput.evaluate(input => input.value);
  //   const displayStyle = await page.evaluate(popup => popup.style.display, popup);
  ...
  //   expect(displayStyle).toBe('none'); // Assuming the component sets display to 'none'
  });
