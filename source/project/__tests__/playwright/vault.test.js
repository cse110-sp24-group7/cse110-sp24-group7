...
async function filterFiles(page, value) {
	...
	for(let i = 1; i < possibleOptions; i++) {
		const isHidden = await page.$eval(`#${possibleOptions[i]}`, el => el.classList.contains("hidden"));
		if(value == 'all') {
...
export default { addFile, removeFile, navigateToVault, deleteAllFiles, filterFiles };