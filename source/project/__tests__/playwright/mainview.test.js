...
function getPosition(string, subString, postion) {
  return string.split(subString, postion).join(subString).length;
}

const getCurrentDate = () => {
  const date = new Date(Date.now()).toISOString();
  const index = getPosition(date, ':', 1);
  return date.substring(0, index + 3);
}
...
test('should open and interact with the TaskPopup and JournalPopup', async () => {
  ...
  await window.fill('task-popup #dueDate', currentTime);
  ...
  expect(addedTask.due_date).toBe(getCurrentDate());
...
//   expect(await journalPopup.isVisible()).toBeTruthy();
  ...
  await window.fill('journal-popup #dueDate', currentTime);
  ...
  await window.click(`.task-pv[data-task-id="${addedTask.task_id}"] .edit-task`);
  ...
  const updatedTask = updatedTasks.find(task => task.task_id === addedTask.task_id);
  ...
  // Delete the Task
  await window.click(`.task-pv[data-task-id="${updatedTask.task_id}"] .delete`);
  await window.dblclick(`.task-pv[data-task-id="${updatedTask.task_id}"] .delete`);
  ...
  const deletedTask = finalTasks.find(task => task.task_id === updatedTask.task_id);
  expect(deletedTask).toBeFalsy();

  await window.click(`.journal-pv[data-entry-id="${addedJournal.entry_id}"] .edit-entry`);
  await window.fill('journal-popup #title', 'Updated Journal');
  await window.fill('journal-popup #description', 'Updated Journal Description');
  ...
  const updatedJournal = updatedJournals.find(entry => entry.entry_id === addedJournal.entry_id);
  ...
  await window.click(`.journal-pv[data-entry-id="${updatedJournal.entry_id}"] .delete`);
  await window.dblclick(`.journal-pv[data-entry-id="${updatedJournal.entry_id}"] .delete`);
  ...
  const deletedJournal = finalJournals.find(entry => entry.entry_id === updatedJournal.entry_id);
