...
describe("Database functions", () => {
    ...
    test('Testing conjunctive label search', done => {
        function trcbConjunctiveTest(tasks) {
            ...
            let idMatch = false;
            if(tasks[0].task_id == test1.task_id && tasks[1].task_id == test3.task_id){
                idMatch = true;
            }
            if(tasks[0].task_id == test3.task_id && tasks[1].task_id == test1.task_id){
                idMatch = true;
            }
            expect(idMatch).toBe(true);
            done();
        }
        dbMgr.getTasksConjunctLabels(["test_label_1", "test_label_2"], trcbConjunctiveTest);
    });
    ...
    test('Testing disjunctive label search', done => {
        function trcbDisjunctiveTest(tasks) {
            ...
            expect(task_ids.sort()).toEqual([test1.task_id, test2.task_id, test3.task_id].sort());
