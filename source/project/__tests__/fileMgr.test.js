...
describe("File functions", () => {
	...
	test("Add all files", () => {
		...
		expect(
			manager.getFileNames().length + manager.getImageNames().length
		).toBe(files.length);
	});
	...
	test("Check that added file contents are the same as the file copied", () => {
		...
		for (let i = 0; i < files.length; i++) {
			...
			const copied_file_location = manager.getFileLocation(
				files[i].name,
				is_img
			);
			const original_file_location = path.join(
				source_location,
				files[i].name
			);

			const copied_file_buffer = fs.readFileSync(copied_file_location);
			const original_file_buffer = fs.readFileSync(
				original_file_location
			);

			if (!copied_file_buffer.equals(original_file_buffer))
				doesPass = false;
		}
	...
	test("Retrieve names of all non-image files", () => {
		const non_image_files = files
			.filter((file) => file.type != "image")
			.map((file) => file.name);
		const retrieved_non_image_files = manager.getFileNames();

		expect(non_image_files.sort().join(",")).toEqual(
			retrieved_non_image_files.sort().join(",")
		);
	});
	...
	test("Retrieve names of all image files", () => {
		...
		expect(image_files.sort().join(",")).toEqual(
			retrieved_image_files.sort().join(",")
		);
	});
