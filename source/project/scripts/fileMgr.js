...
const FileManager = (data_location) => {
    ...
    */
	const add = (file) => {
		...
		if (fs.existsSync(fileLocation)) throw new Error('A file with that name already exists')
		try {
    ...
    */
	const remove = (file_name, is_img = false) => {
		...
		if (!fs.existsSync(fileLocation)) throw new Error(`A file with the name ${file_name} was not found. No files were deleted.`)
